          ...
          <div className="space-y-3 mt-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="rounded-xl border border-blue-500/30 bg-blue-500/5 p-3 space-y-2">
                <p className="text-[11px] font-black text-blue-300 uppercase tracking-widest">🔓 Public Key (N, e)</p>
              ...
              <div className="rounded-xl border border-rose-500/30 bg-rose-500/5 p-3 space-y-2">
                <p className="text-[11px] font-black text-rose-300 uppercase tracking-widest">🔐 Private Key (N, d)</p>
        ...
        <div className="flex gap-2">
          {['textbook', 'pkcs15'].map(m => (
            <button key={m} type="button" onClick={() => setMode(m)}
              ...
              {m === 'textbook' ? '📖 Textbook RSA' : '🛡 PKCS#1 v1.5'}
            </button>
          ))}
        </div>
        {mode === 'textbook' && (
          <div className="rounded-xl border border-rose-500/30 bg-rose-500/5 px-3 py-2 text-xs text-rose-300">
            ⚠ <strong>Textbook RSA is deterministic</strong> — the same plaintext always produces the same ciphertext.
            This breaks CPA security. See the Determinism tab for the full attack demo.
          </div>
        )}
      ...
      <Card title="Textbook RSA Determinism Attack">
        ...
        <div className="flex gap-3 flex-wrap items-end">
          ...
          <Btn onClick={run} disabled={loading || !keys} id="pa12-det-btn">
            {loading ? <><Spinner />Running…</> : '⚡ Encrypt Twice'}
          </Btn>
        </div>
        {!keys && <p className="text-xs text-amber-400 italic mt-2">Generate keys first (🔑 tab)</p>}
      ...
      {result && (
        <>
          ...
          <div className={cx(
            ...
            {tb?.identical && (
              <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 px-4 py-3 text-center">
                <p className="text-rose-300 font-black text-sm">🚨 C₁ = C₂ — An attacker observing both ciphertexts knows they encrypt the same plaintext!</p>
          ...
          <div className={cx(
            ...
            <div className="flex items-center gap-3">
              <span className="text-lg font-black">🛡 PKCS#1 v1.5 RSA</span>
              {!pk?.identical
            ...
            {!pk?.identical && (
              <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-center">
                <p className="text-emerald-300 font-black text-sm">✅ C₁ ≠ C₂ — Random PS bytes make each encryption unique, achieving CPA security.</p>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-3 flex-wrap items-end">
            ...
            <Btn onClick={run} disabled={loading} id="pa12-bb-btn">
              {loading ? `Running… (${elapsed}s)` : '🔓 Run Oracle Demo'}
            </Btn>
          </div>
          {error && <p className="text-xs text-rose-400 font-black">{error}</p>}
        </div>
